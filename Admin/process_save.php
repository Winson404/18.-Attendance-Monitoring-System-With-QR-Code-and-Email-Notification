...
if(isset($_POST['create_grade'])) {
	...
	if(isset($_POST['save_assign_teacher'])) {
		...
		$plus = strtotime('+25 minutes', $time);
		$result = date("H:i", $plus);

		$absent_time = strtotime($startime);
		$absent_plus = strtotime('+40 minutes', $absent_time);
		$absent_result = date("H:i", $absent_plus);

		$endtime        = $_POST['endtime'];
		$day            = $_POST['day'];

		$fetch = mysqli_query($conn, "SELECT * FROM assign WHERE assign_user_Id='$teacher' AND assign_subject_Id='$subject' AND assign_section_Id='$strand_section' AND start_time='$startime' AND end_time='$endtime' AND day='$day'");
		if(mysqli_num_rows($fetch) > 0) {
        ...
        $_SESSION['status'] = "error";
			...
			$fetch = mysqli_query($conn, "SELECT * FROM assign WHERE assign_user_Id='$teacher' AND start_time='$startime' AND end_time='$endtime' AND day='$day'");
			if(mysqli_num_rows($fetch) > 0) {
					$_SESSION['message'] = "Teacher has already been assigned with this date and time.";
	        $_SESSION['text'] = "Please try again.";
	        $_SESSION['status'] = "error";
					header("Location: assigning.php");
			} else {
					$save = mysqli_query($conn, "INSERT INTO assign (assign_user_Id, assign_subject_Id, assign_section_Id, start_time, end_time, day, allowance_time, absent_time) VALUES ('$teacher', '$subject', '$strand_section', '$startime', '$endtime', '$day', '$result', '$absent_result')");
					if($save) {
	...
	if(mysqli_num_rows($fetch) > 0) {
		...
		if($sec_Id != $section_Id) {
		    ...
		    $_SESSION['status'] = "error";
			    ...
			    $_SESSION['text'] = "Please try again.";
			    $_SESSION['status'] = "error";
			    $script = "<script>$(window).on('load', function() { $('#add_user').modal('show'); load_upmodal(); });</script>";
					header('Location: attendance.php?section_Id='.$sec_Id.'&&assignId='.$assignId.'&&script='.$script.'');
				} else {
...
// THE PURPOSE OF THIS IS NOT TO SAVE
// THIS IS JUST TO GET THE ASSIGN_ID 
// AND REDIRECT PAGE WITH A GET METHOD 
// INTO ATTENDANCE.PHP
if(isset($_POST['getsection_Id_And_Assign_Id'])) {
	$section_Id = $_POST['section_Id'];
	$assignId   = $_POST['assignId'];

	
	header('Location: attendance.php?section_Id='.$section_Id.'&&assignId='.$assignId.' ');
}
?>
