...
if(isset($_POST['create_grade'])) {
	...
	if(isset($_POST['save_assign_teacher'])) {
		...
		$startTimes = date("H:i", strtotime('-30 minutes', $startime));
		$endTimes = date("H:i", strtotime('+30 minutes', $time));
        ...
        $_SESSION['status'] = "error";
			...
			$save = mysqli_query($conn, "INSERT INTO assign (assign_user_Id, assign_subject_Id, assign_section_Id, start_time, end_time, day) VALUES ('$teacher', '$subject', '$strand_section', '$startTimes', '$endtime', '$day')");
			if($save) {
...
if(isset($_POST['lrn'])) {
	...
	$get_timeStart = mysqli_query($conn, "SELECT * FROM assign WHERE assign_section_Id='$sec_Id' ");
	$r_timeStart = mysqli_fetch_array($get_timeStart);
	$sec_timeStart = strtotime($r_timeStart['start_time']);	
	
	date_default_timezone_set('Asia/Manila');

		$time = date('H:i');
		$minus = strtotime('-30 minutes', strtotime($time));
		$result = date("H:i", $minus);
		...
		if($sec_Id != $section_Id) {
		    ...
		    $_SESSION['status'] = "error";
				...
				// } else {
						...
						$save = mysqli_query($conn, "INSERT INTO attendance (attendance_student_Id, attendance_section_Id, time_in, logdate, remarks) VALUES ('$student_Id', '$section_Id', '$result', '$date', '$remarks')");
						if($save) {
							$_SESSION['message'] = ' '.$row['firstname'].' '.$row['middlename'].' '.$row['lastname'].' '.$row['suffix'].' ' ;
					    $_SESSION['text'] = 'Successfully Timed in.'.$result.' ';
					    $_SESSION['status'] = "success";
							header('Location: attendance.php?section_Id='.$sec_Id.'');
						} else {
